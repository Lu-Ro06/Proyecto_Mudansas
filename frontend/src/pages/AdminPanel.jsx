...
const AdminPanel = () => {
    ...
    const fetchData = async () => {
        try {
            const [resProv, resSol] = await Promise.all([
                fetch('http://localhost:5000/api/admin/proveedores'),
    ...
    const handleCreateUser = async (e) => {
        ...
        setMessage('');
        try {
            const response = await fetch('http://localhost:5000/api/admin/create_user', {
                method: 'POST',
    ...
    const handleAsignar = async (id_solicitud, id_proveedor) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/admin/solicitudes/${id_solicitud}/asignar`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
